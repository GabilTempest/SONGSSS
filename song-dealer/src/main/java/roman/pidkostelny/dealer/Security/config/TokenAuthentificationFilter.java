package roman.pidkostelny.dealer.Security.config;
...
public class TokenAuthentificationFilter extends GenericFilterBean {
    ...
    @Value("${external.url.pattern.Rock}")
    private String BandsPattern;

    @Value("${external.url.pattern.Songs}")
    private String SongsPattern;

  @Value("${external.static.MP3_DIR.pattern}")
    private String MP3_DIRPattern;



    @Value("${external.url.pattern.sing}")
    private String singPattern;

    @Value("${external.url.pattern.Band}")
    private String BandPattern;


    @Value("${external.url.pattern.DemoPlayer}")
    private String DemoPlayerPattern;

    @Value("${external.url.pattern.Genre}")
    private String GenrePattern;

    @Value("${external.url.pattern.SongsIndex}")
    private String SongsIndexPattern;

    @Value("${external.url.pattern.reGestracja}")
    private String reGestracjaPattern;
    ...
    public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain) throws IOException, ServletException {
        ...
        List<String> urlPatterns = Arrays.asList(externalUrlPattern,
                homePattern, singPattern, imagesUrlPattern, cssUrlPattern,SongsPattern,
                 BandPattern, BandPattern, reGestracjaPattern, DemoPlayerPattern,MP3_DIRPattern,
                SongsIndexPattern, GenrePattern, "/v2/api-docs", "/configuration/ui", "/swagger-resources", "/configuration/security", "/swagger-ui.html", "/webjars/*","/mp3/*");
